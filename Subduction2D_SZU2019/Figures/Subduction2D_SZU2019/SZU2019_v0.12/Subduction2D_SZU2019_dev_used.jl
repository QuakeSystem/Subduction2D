...
function main(li, origin, phases_GMG, igg; nx=16, ny=16, figdir="figs2D", do_vtk=false)
    ...
    # for j in 3:ny-5#28
    #     push!(nodes_boundary_box, round(nx * 0.2) + j * nx)
    #     push!(nodes_boundary_box, round(nx * 0.2) + 1 + j * nx)
    #     push!(nodes_boundary_box, round(nx * 0.2) + 2 + j * nx)
    # end
    # Boundary conditions
    flow_bcs = VelocityBoundaryConditions(; # JustRelax.jl/src/boundaryconditions/types.jl
        free_slip=(left=true, right=true, top=true, bot=true),
        free_surface=false,
        # custom_slip=nodes_boundary_box,
    )
...
figdir = "Subduction2D_SZU2019/Figures/Subduction2D_SZU2019/SZU2019_v0.12"
n = 32
nx, ny = n * 2, n
