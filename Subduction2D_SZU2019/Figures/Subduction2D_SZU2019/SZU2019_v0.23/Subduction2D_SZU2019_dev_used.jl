...
function main(li, origin, phases_GMG, igg; nx=16, ny=16, figdir="figs2D", do_vtk=false)
    ...
    rheology = init_rheologies() # JustRelax.jl/miniapps/subduction/2D/Subduction2D_rheology.jl
    dt = 0.5 * 10.0e3 * 3600 * 24 * 365 # diffusive CFL timestep limiter
    # ----------------------------------------------------
    ...
    # Is xnode 30 - 32, ynode 18 - 28.
    nodes_boundary_box = Int[]

    # for nx=128 * 256
    # for j in 26:50#28
    #     push!(nodes_boundary_box, 30 + j * nx)
    #     push!(nodes_boundary_box, 31 + j * nx)
    #     push!(nodes_boundary_box, 32 + j * nx)
    # end

    # horizontal_node = Int16(round(nx * 0.2))
    # for j in 10:ny-2
    #     index_next_row = j * nx
    #     new_node = horizontal_node + index_next_row
    #     push!(nodes_boundary_box, horizontal_node + index_next_row)
    #     println("node:", new_node, "coordinate:", xci[1][new_node/ny], xci[2][new_node/nx])
    #     # push!(nodes_boundary_box, round(nx * 0.2) + 1 + index_next_row)
    #     # push!(nodes_boundary_box, round(nx * 0.2) + 2 + index_next_row)
    # end
    # println("horizontal_nodes:", horizontal_node)
    # println("nodes with booster:", nodes_boundary_box)

    # horizontal_node = Int16(round(ny * 0.2))
    # for i in 10:12
    #     index_next_row = i * ny
    #     push!(nodes_boundary_box, horizontal_node + index_next_row)
    # print
    ...
    # push!(nodes_boundary_box, round(nx * 0.2) + 2 + index_next_row)
    # # end
    for i in (ny+2)-10:(ny+2)-5
        push!(nodes_boundary_box, 12 + i * (nx + 1))
        push!(nodes_boundary_box, 13 + i * (nx + 1))
        push!(nodes_boundary_box, 14 + i * (nx + 1))
        push!(nodes_boundary_box, 15 + i * (nx + 1))
        push!(nodes_boundary_box, 16 + i * (nx + 1))
        push!(nodes_boundary_box, 17 + i * (nx + 1))
        println("xcoord, y coord:", grid_vxi[1][1][12], grid_vxi[1][2][i])
        # println("xcoord, y coord:", grid_vxi[1][1][13], grid_vxi[1][2][i])
        # println("xcoord, y coord:", grid_vxi[1][1][14], grid_vxi[1][2][i])
    end
    # println("node:", new_node, "coordinate:", xci[1][new_node], xci[2][new_node/nx])
    # column = 4
    # global_index = 4 + 1 * nx
    # println("dims nx ny:", nx, " ", ny)
    # println("coordinate node 4:", xci[1][4], xci[2][4])
    ...
    flow_bcs = VelocityBoundaryConditions(; # JustRelax.jl/src/boundaryconditions/types.jl
        ...
        free_surface=false,
        custom_slip=nodes_boundary_box, # hardcoded 7.5cm/yr convergence.
    )
    ...
    while it < 500 # run only for 5 Myr
        ...
        if it == 1 || rem(it, 10) == 0
            ...
            ax1 = Axis(fig[1, 1], aspect=ar, title="Vx [m/s]  (t=$(t / (1.0e6 * 3600 * 24 * 365.25)) Myrs)")
            ...
            # --- ZOOM REGION ---
            xmin, xmax = 0, 1500     # km
            zmin, zmax = -200, 10     # km depth (negative)
            ...
            h1 = heatmap!(ax1, xvi[1] .* 1.0e-3, xvi[2] .* 1.0e-3, Array(stokes.V.Vx[2:(end-1), :]), colormap=:batlow)
...
figdir = "Subduction2D_SZU2019/Figures/Subduction2D_SZU2019/SZU2019_v0.23"
n = 32 * 2
nx, ny = n * 2, n
...
for f in script_files
    ...
    name, ext = splitext(f)
    dest = joinpath(figdir, name * suffix * ext)
