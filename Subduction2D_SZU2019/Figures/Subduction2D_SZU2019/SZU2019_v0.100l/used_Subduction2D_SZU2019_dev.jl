...
function main(li, origin, phases_GMG, igg; nx=16, ny=16, figdir="figs2D", do_vtk=false)
    ...
    for i in (ny+2)-37:(ny+2)-27
        ...
        println("xcoord, y coord:", grid_vxi[1][1][33], grid_vxi[1][2][i])
    ...
    while it < 500 # run only for 5 Myr
        ...
        # Make Makie figure
        fig = Figure(size=(1500, 600), title="t = $t_Myr_round, dt = $t_Kyr Kyr")
        Label(
            fig[0, :],
        ...
        ax6 = Axis(fig[3, 3], aspect=ar, title="τII [Pa]")
        ...
        zmin, zmax = -80, 30     # km depth (negative)
        ...
        h1 = scatter!(ax1, Array(pxv[idxv]), Array(pyv[idxv]), color=Array(clr[idxv]), markersize=1)

        # Plot temperature
        h2 = heatmap!(ax2, xvi[1] .* 1.0e-3, xvi[2] .* 1.0e-3, Array(thermal.T[2:(end-1), :]), colormap=:vik)
        ...
        h6 = heatmap!(ax6, xci[1] .* 1.0e-3, xci[2] .* 1.0e-3, Array((stokes.τ.II)), colormap=:batlow)

        # Plot 2nd invariant of strain rate
        h7 = heatmap!(ax7, xci[1] .* 1e-3, xci[2] .* 1e-3, Array(log10.(stokes.ε.II)), colormap=:vik)
...
figdir = "Subduction2D_SZU2019/Figures/Subduction2D_SZU2019/SZU2019_v0.100l"
n = 32 * 2 * 2
nx, ny = n * 2, n
