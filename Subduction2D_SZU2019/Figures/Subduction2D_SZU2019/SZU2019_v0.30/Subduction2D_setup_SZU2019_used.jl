...
function GMG_subduction_2D(nx, ny)
    ...
    add_box!(
        ...
        phase=LithosphericPhases(Layers=[], Phases=[0]), # √GeophysicalModelGenerator.jl/src/Setup_geometry.jl
        # T=HalfspaceCoolingTemp(Tsurface=20, Tmantle=Tbot, Age=50, Adiabat=0)
    )
    # 2: air
    ...
    add_box!(
        ...
        Origin=nothing, StrikeAngle=0, DipAngle=0,
        phase=LithosphericPhases(Layers=[], Phases=[1]), # √GeophysicalModelGenerator.jl/src/Setup_geometry.jl
        # T=HalfspaceCoolingTemp(Tsurface=20, Tmantle=Tbot, Age=50, Adiabat=0)
    )
    ...
    add_polygon!(
        ...
        phase=LithosphericPhases(Layers=[2 5], Phases=[4 5], Tlab=Tlab), # √GeophysicalModelGenerator.jl/src/Setup_geometry.jl
